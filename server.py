...
class WebAnalyzer:
    ...
    async def check_vip_link(self, link):
        """检查VIP链接"""
        try:
            # 获取重定向URL
            redirect_url = await self.get_redirect_url(link)
    ...
    async def check_gift_link(self, link):
        """检查礼品卡链接"""
        try:
            # 如果有原始分析器，优先使用
    ...
    async def analyze_single_link(self, link):
        """分析单个链接"""
        try:
            # 检查链接类型
            redirect_url = await self.get_redirect_url(link)
            ...
            if 'vip-invite-cashier' in redirect_url:
                return await self.check_vip_link(link)
            elif '163cn.tv' in link:
                return await self.check_gift_link(link)
            else:
                return {
                    'short_url': link,
                    'status': 'error',
...
def analyze_link():
    ...
    try:
        ...
        try:
            result = loop.run_until_complete(analyzer.analyze_single_link(link))
...
def batch_analyze():
    ...
    try:
        ...
        if len(links) > 100:  # 限制批量数量
            return jsonify({
                'error': '单次最多支持100个链接'
            }), 400
        
        max_workers = min(data.get('max_workers', 5), 10)  # 限制并发数
        
        async def batch_process():
            results = []
            semaphore = asyncio.Semaphore(max_workers)
            
            async def analyze_with_semaphore(link):
                async with semaphore:
                    return await analyzer.analyze_single_link(link)
            
            tasks = [analyze_with_semaphore(link) for link in links]
